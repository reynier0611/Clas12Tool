...
void HallBEventRecordMT(){
  ...
  //ignore this just getting file name!
   ...
   Int_t Nworkers=1;
   ROOT::TThreadExecutor pool(Nworkers);
 
   //for(Int_t i=0;i<files->GetEntries();i++){
   hipo::reader  reader;
   vector<hipo::reader* > readers;
   vector<hallB_event* > events;
   vector<std::pair<int,int>> ranges;
   for(int i=0;i<Nworkers;i++){
     auto reader=new hipo::reader();
     reader->open(files->At(0)->GetTitle());
     readers.push_back(reader);
     events.push_back(new hallB_event(*reader));
     if(i==0){
       Int_t Nrecords=reader->getNRecords();
       Int_t NrecPerWorker= NrecPerWorker=Nrecords/(Nworkers);      
       for(Int_t j=0;j<Nworkers;j++)
	 if(j!=Nworkers-1)
	   ranges.push_back(std::make_pair<Int_t,Int_t>(j*NrecPerWorker,(j+1)*NrecPerWorker));
	 else 
	   ranges.push_back(std::make_pair<Int_t,Int_t>(j*NrecPerWorker,1*Nrecords));
			  
     }
   }
   auto playRecords = [&readers,&events,&ranges](int iread = 0){
     int counter=0;

     auto reader= readers[iread];
     auto event=events[iread];
     std::chrono::nanoseconds dura( 100);

     for(Int_t irepeat=0;irepeat<100;irepeat++)
     for(Int_t irec=ranges[iread].first;irec<ranges[iread].second;irec++){
   ...
   auto ibins = ROOT::TSeqI(Nworkers);
   pool.Map(playRecords, ibins);
